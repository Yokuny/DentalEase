...
const Anamnesis = ({ toast }: ToastProps) => {
  ...
  const anamnesisParam = searchParams.get("interface");

  const router = useRouter();
  const { id } = useParams();

  useEffect(() => {
    anamnesisParam === "anamnesis" ? setOpen(true) : setOpen(false);
  }, [anamnesisParam]);
  ...
  async function onSubmit(values: z.infer<typeof anamnesisSchema>) {
    ...
    const body = {
      ...
      pregnancyMonth: Number(values.pregnancyMonth),
      ...
      <DialogContent className="md:p-6 p-0 pb-0 justify-center flex flex-col">
        ...
        <ScrollArea className="w-full h-[70vh] pr-4">
          <Form {...form}>
            <form
              id="anamnesis-form"
              className="mb-1 gap-4 flex-col flex w-full"
              onSubmit={(e) => {
                e.preventDefault();
