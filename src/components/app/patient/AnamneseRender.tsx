...
const Title = ({ children }: Node) => (
  <h3 className="mb-2 text-sm font-medium tracking-wide text-muted-foreground">{children}</h3>
);
const Subtitle = ({ children }: Node) => <h4 className="text-sm tracking-wide text-muted-foreground">{children}</h4>;
const Content = ({ children }: Node) => <p className="pl-2 text-sm gap-2 flex">{children}</p>;

const LiItem = ({ children }: Node) => (
  <li className="gap-2 items-center flex">
    <Content>{children}</Content>
  </li>
);

const AnamneseRender = ({ anamnese }: { anamnese: Anamnese }) => {
  const hasEspecialConditions =
    anamnese.allergicToMedication ||
    anamnese.medicationAllergy ||
    anamnese.gumsBleedEasily ||
    anamnese.sensitiveTeeth ||
    anamnese.pregnant ||
    anamnese.pregnancyMonth ||
    anamnese.breastfeeding ||
    anamnese.underMedicalTreatment ||
    anamnese.medicalTreatmentDetails ||
    anamnese.takingMedication ||
    anamnese.medicationDetails;

  return (
    <div className="gap-2 w-full whitespace-nowrap flex-wrap flex-row flex">
...
function CigaretteIcon(props: any) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
